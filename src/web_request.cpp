	...
	headers_(nullptr)
{
	...
	curl_easy_setopt(curl_ptr_, CURLOPT_WRITEFUNCTION, WebRequest::curl_write_callback);
...
void WebRequest::set_receive_callback(std::function<void(const std::string &)> receive_callback)
{
    receive_callback_ = receive_callback;
}

void WebRequest::set_error_callback(std::function<void(const std::string &)> error_callback)
{
    error_callback_ = error_callback;
}

bool WebRequest::execute(http::Method method, const std::string &url, const std::string &message)
{
    if (curl_ptr_)
    {
        std::string response_data;

        curl_easy_setopt(curl_ptr_, CURLOPT_CUSTOMREQUEST, http::method_to_string(method).c_str());
        curl_easy_setopt(curl_ptr_, CURLOPT_URL, url.c_str());
		
		if(method == http::Method::METHOD_PUT && !message.empty())
...
void WebRequest::set_timeout(std::uint16_t seconds)
{
    if (curl_ptr_)
        curl_easy_setopt(curl_ptr_, CURLOPT_TIMEOUT, seconds);
}

void WebRequest::set_ssl_verification(bool value)
{
    if (value)
        if (curl_ptr_)
...
void WebRequest::set_http_header(std::initializer_list<std::string> header_parameters)
{
    if(headers_)
        curl_slist_free_all(headers_);
...
std::size_t WebRequest::curl_write_callback(const char* in, std::size_t size, std::size_t num, std::string* out)
{
